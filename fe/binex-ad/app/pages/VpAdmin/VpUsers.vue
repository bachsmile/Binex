...
const fetchUsers = async () => {
  loading.value = true;
  try {
    const response = await userApi.findMine({
      page: page.value,
      limit: limit.value,
  ...
  <div class="flex-1 flex flex-col min-w-0">
    ...
    <section class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
      <div>
        ...
        <p class="text-xs sm:text-sm text-zinc-450 dark:text-zinc-500 font-medium mt-2">
        ...
        <template #actions="{ item: user }">
          <div class="inline-flex items-center gap-2">
            <CmButton 
              variant="outline" 
              icon="heroicons:eye" 
              class="h-8 text-[11px] border-zinc-150 dark:border-zinc-800 font-bold px-3 py-1 hover:bg-zinc-50 dark:hover:bg-zinc-900 text-zinc-650 dark:text-zinc-300"
